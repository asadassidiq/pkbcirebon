...
class KendaraanRangeExport implements FromView
{
    use Exportable;
    ...
    public function view(): View
    {
        ...
        $kendaraan  = Pendaftaran::leftJoin('identitaskendaraans','identitaskendaraans.id','=','pendaftarans.identitaskendaraan_id')->leftJoin('kodepenerbitans','kodepenerbitans.id','=','pendaftarans.kodepenerbitans_id')->Join('pengujians','pengujians.pendaftaran_id','=','pendaftarans.id')->leftJoin('transaksis','transaksis.pendaftaran_id','=','pendaftarans.id')->where('kodepenerbitans_id','!=','7')->where('kodepenerbitans_id','!=','12')->where('kodepenerbitans_id','!=','9')->where('kodepenerbitans_id','!=','10')->where('kodepenerbitans_id','!=','3')->where('pendaftarans.status','1')->whereBetween('tglpendaftaran',[$this->tgl1,$this->tgl2])->groupBy('identitaskendaraan_id')->get();

        return view('exports.range.kendaraan', ['kendaraan' => $kendaraan,'tglprint' => $tglprint]);
    }
}
