...
class TidakLulusRangeExport implements FromView
{
    use Exportable;
    ...
    public function view(): View
    {
        ...
        $tidaklulus = Pendaftaran::leftJoin('identitaskendaraans','identitaskendaraans.id','=','pendaftarans.identitaskendaraan_id')->Join('pengujians','pengujians.pendaftaran_id','=','pendaftarans.id')->where('pendaftarans.status','1')->whereBetween('tglpendaftaran',[$this->tgl1,$this->tgl2])->where('kodepenerbitans_id','!=','7')->where('kodepenerbitans_id','!=','12')->where(function ($tidaklulus){ $tidaklulus->where('statuslulusuji','0')->orWhereNull('statuslulusuji');})->get();

        return view('exports.range.tidaklulus', ['tidaklulus' => $tidaklulus,'tglprint' => $tglprint]);
    }
}
