    ...
    public function setCopyPengujian($id)
    {
        ...
        if ($data) {
            ...
            $bagianbawahkendaraan = BagianBawahKendaraan::Create([
                'pendaftaran_id'    => $id,
                'sistemkemudi'      => $bagianbawahkendaraan->sistemkemudi ?? 1,
                'sistemsuspensi'    => $bagianbawahkendaraan->sistemsuspensi ?? 1,
                'sistemrem'         => $bagianbawahkendaraan->sistemrem ?? 1,
                'sistemroda'        => $bagianbawahkendaraan->sistemroda ?? 1,
                'rangkalandasan'    => $bagianbawahkendaraan->rangkalandasan ?? 1,
                'sistempenerusdaya' => $bagianbawahkendaraan->sistempenerusdaya ?? 1,
                'motorpenggerak'    => $bagianbawahkendaraan->motorpenggerak ?? 1,
            ]);
            ...
            $bagianbelakangkendaraan = BagianBelakangKendaraan::Create([
                'pendaftaran_id'    => $id,
                'apctbelakang'      => $bagianbelakangkendaraan->apctbelakang ?? 1,
                'lampuposisibelakang'    => $bagianbelakangkendaraan->lampuposisibelakang  ?? 1,
                'lampupenunjukarah'      => $bagianbelakangkendaraan->lampupenunjukarah ?? 1,
                'lampurem'        => $bagianbelakangkendaraan->lampurem ?? 1,
                'lampumundur'    => $bagianbelakangkendaraan->lampumundur ?? 1,
                'lamputnkb' => $bagianbelakangkendaraan->lamputnkb ?? 1,
                'kondisitempatpemasangantnkb'    => $bagianbelakangkendaraan->kondisitempatpemasangantnkb ?? 1,
                'lamputandabatas'    => $bagianbelakangkendaraan->lamputandabatas ?? 1,
                'kondisirodacadangan'    => $bagianbelakangkendaraan->kondisirodacadangan ?? 1,
                'spakbor'    => $bagianbelakangkendaraan->spakbor ?? 1,
                'sistempembuangan'    => $bagianbelakangkendaraan->sistempembuangan ?? 1,
            ]);
            ...
            $bagiandalamkendaraan = BagianDalamKendaraan::Create([
                'pendaftaran_id'    => $id,
                'ruangkemudi'       => $bagiandalamkendaraan->ruangkemudi ?? 1,
                'tempatduduk'       => $bagiandalamkendaraan->tempatduduk ?? 1,
                'dashboard'         => $bagiandalamkendaraan->dashboard ?? 1,
                'speedometer'       => $bagiandalamkendaraan->speedometer ?? 1,
                'instrumenpanel'    => $bagiandalamkendaraan->instrumenpanel ?? 1,
                'alatcontrollampu'  => $bagiandalamkendaraan->alatcontrollampu ?? 1,
                'tabirmatahari'     => $bagiandalamkendaraan->tabirmatahari ?? 1,
                'pandangankedepan'  => $bagiandalamkendaraan->pandangankedepan ?? 1,
                'rodakemudi'        => $bagiandalamkendaraan->rodakemudi ?? 1,
                'batangkemudi'      => $bagiandalamkendaraan->batangkemudi ?? 1,
                'sabukkeselamatan'  => $bagiandalamkendaraan->sabukkeselamatan ?? 1,
                'perlengkapan'      => $bagiandalamkendaraan->perlengkapan ?? 1,
                'remparkir'         => $bagiandalamkendaraan->remparkir ?? 1,
                'pedalremkaki'      => $bagiandalamkendaraan->pedalremkaki ?? 1,
                'remgasbuang'       => $bagiandalamkendaraan->remgasbuang ?? 1,
                'sistemhampa'       => $bagiandalamkendaraan->sistemhampa ?? 1,
            ]);
            ...
            $bagiandepankendaraan = BagianDepanKendaraan::Create([
                'pendaftaran_id'    => $id,
                'kacadepan'         => $bagiandepankendaraan->kacadepan ?? 1,
                'penyemprotair'     => $bagiandepankendaraan->penyemprotair ?? 1,
                'penghapuskaca'     => $bagiandepankendaraan->penghapuskaca ?? 1,
                'bodidepan'         => $bagiandepankendaraan->bodidepan ?? 1,
                'lamputandabatas'   => $bagiandepankendaraan->lamputandabatas ?? 1,
                'lampuutamadekat'   => $bagiandepankendaraan->lampuutamadekat ?? 1,
                'lampuutamajauh'    => $bagiandepankendaraan->lampuutamajauh ?? 1,
                'lampupenunjukarah' => $bagiandepankendaraan->lampupenunjukarah ?? 1,
                'lampuposisidepan'  => $bagiandepankendaraan->lampuposisidepan ?? 1,
                'lampukabut'        => $bagiandepankendaraan->lampukabut ?? 1,
                'bumperdepan'       => $bagiandepankendaraan->bumperdepan ?? 1,
                'kondisitempatnkbdepan'        => $bagiandepankendaraan->kondisitempatnkbdepan ?? 1,
            ]);
            ...
            $bagiankanankendaraan = BagianKananKendaraan::Create([
                'pendaftaran_id'    => $id,
                'kacasampingkanan'  => $bagiankanankendaraan->kacasampingkanan ?? 1,
                'kacaspion'         => $bagiankanankendaraan->kacaspion ?? 1,
                'pintusampingkanan' => $bagiankanankendaraan->pintusampingkanan ?? 1,
                'bodikanan'         => $bagiankanankendaraan->bodikanan ?? 1,
                'apctkanan'         => $bagiankanankendaraan->apctkanan ?? 1,
                'kompresorudara'    => $bagiankanankendaraan->kompresorudara ?? 1,
                'bautdanmurroda'    => $bagiankanankendaraan->bautdanmurroda ?? 1,
                'kondisidanukuranban'    => $bagiankanankendaraan->kondisidanukuranban ?? 1,
                'bautdanmurroda'    => $bagiankanankendaraan->bautdanmurroda ?? 1,
                'perisaikolong'     => $bagiankanankendaraan->perisaikolong ?? 1,
                'tutuptangki'       => $bagiankanankendaraan->tutuptangki ?? 1,
                'lamputandabatassamping'       => $bagiankanankendaraan->lamputandabatassamping ?? 1,
                'apar'              => $bagiankanankendaraan->apar ?? 1,
                'rodakelima'        => $bagiankanankendaraan->rodakelima ?? 1,
                'kakipenompangtempelan'        => $bagiankanankendaraan->kakipenompangtempelan ?? 1,
                'alatperangkaitempelan'        => $bagiankanankendaraan->alatperangkaitempelan ?? 1,
            ]);
            ...
            $bagiankirikendaraan = BagianKiriKendaraan::Create([
                'pendaftaran_id'    => $id,
                'kacasampingkiri'  => $bagiankirikendaraan->kacasampingkiri ?? 1,
                'kacaspionkiri'     => $bagiankirikendaraan->kacaspionkiri ?? 1,
                'pintusampingkiri'  => $bagiankirikendaraan->pintusampingkiri ?? 1,
                'bodikiri'          => $bagiankirikendaraan->bodikiri ?? 1,
                'apctkiri'          => $bagiankirikendaraan->apctkiri ?? 1,
                'kompresorudara'    => $bagiankirikendaraan->kompresorudara ?? 1,
                'bautdanmurroda'    => $bagiankirikendaraan->bautdanmurroda ?? 1,
                'kondisidanukuranban'    => $bagiankirikendaraan->kondisidanukuranban ?? 1,
                'bautdanmurroda'    => $bagiankirikendaraan->bautdanmurroda ?? 1,
                'perisaikolong'     => $bagiankirikendaraan->perisaikolong ?? 1,
                'tutuptangki'       => $bagiankirikendaraan->tutuptangki ?? 1,
                'lamputandabatassamping'       => $bagiankirikendaraan->lamputandabatassamping ?? 1,
                'rodakelima'        => $bagiankirikendaraan->rodakelima ?? 1,
                'kakipenompangtempelan'        => $bagiankirikendaraan->kakipenompangtempelan ?? 1,
                'alatperangkaitempelan'        => $bagiankirikendaraan->alatperangkaitempelan ?? 1,
            ]);
            ...
            $identifikasikendaraan = IdentifikasiKendaraan::Create([
                'pendaftaran_id'    => $id,
                'notnkb'            => $identifikasikendaraan->notnkb ?? 1,
                'nouji'             => $identifikasikendaraan->nouji ?? 1,
                'norangkalandasan'  => $identifikasikendaraan->norangkalandasan ?? 1,
                'nomotorpenggerak'  => $identifikasikendaraan->nomotorpenggerak ?? 1,
            ]);
