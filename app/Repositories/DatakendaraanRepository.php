...
class DatakendaraanRepository
{
    ...
    public function __construct(Identitaskendaraan $model, Datakendaraan $model1, Pendaftaran $model2,Identitaskendaraan1 $model3)
    {
        $this->model = $model;
        $this->model1 = $model1;
        $this->model2 = $model2;
    ...
    public function riwayatUji($id)
    {
        ...
        if($identitaskendaraan){
            $data = $this->model2->select('pendaftarans.uuid','kodepenerbitans.keterangan','pendaftarans.tglpendaftaran','pendaftarans.pos1','pendaftarans.pos2','pendaftarans.pos3','pendaftarans.posverif','pendaftarans.posverif2','laikjalan.*')->join('kodepenerbitans', 'pendaftarans.kodepenerbitans_id', '=', 'kodepenerbitans.id')->join('laikjalan','laikjalan.pendaftaran_id','pendaftarans.id')->where('identitaskendaraan_id',$identitaskendaraan->id)->orderBy('pendaftarans.tglpendaftaran','ASC')->limit(10)->get();
            return $data;
        }
        return null;
    }
    ...
    public function updateData()
    {
        ...
        foreach($pendaftaran as $pend)
        {
            ...
            $dataPend = Pendaftaran1::create($pendaftaran);
            if($dataPend)
            {
