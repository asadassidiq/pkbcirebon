	...
	<div class="card card-custom">
		<div class="card-header">
			<div class="card-title">
				...
				<h3 class="card-label">Verifikator 1 Tidak Lulus</h3>
			</div>
			...
			<div class="row table-responsive">
				<div class="col-md-12">
					<table class="table">
						...
						<tbody>
							<tr v-for="pendaftaran, index in verif.pendaftaransTLulus.data" :key="pendaftaran.uuid">
								<td>{{ index+1 }}</td>
								<td>{{ pendaftaran.keterangan }}</td>
								<td>{{ pendaftaran.nouji }}</td>
								<td>{{ pendaftaran.noregistrasikendaraan }}</td>
								<td>
									<router-link :to="{ name: 'verif.pengujian', params: { id: pendaftaran.uuid } }">
		...
		<div class="card-footer">
			<div class="pull-right">
				<div class="overflow-auto">
					<b-pagination v-model="page" :total-rows="verif.pendaftaransTLulus.total"
						:per-page="verif.pendaftaransTLulus.per_page" aria-controls="pendaftaransTLulus"
						v-if="verif.pendaftaransTLulus.data && verif.pendaftaransTLulus.data.length > 0"></b-pagination>
				</div>
			</div>
		</div>
	</div>
...
export default {
	...
	methods: {
		...mapActions("verif", ["getPendaftaransTLulus"]),
