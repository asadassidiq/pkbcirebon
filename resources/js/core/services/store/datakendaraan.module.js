...
export const mutations = {
  ...
  ASSIGN_FORM(state, payload) {
    ...
    if (
      payload.kodewilayahasal !== null &&
      payload.kodewilayahasal !== undefined &&
      payload.kodewilayahasal !== ""
    ) {
      var wilayah = state.kodewilayahs.find(function (item) {
    ...
    if (
      payload.merek !== null &&
      payload.merek !== undefined &&
      payload.merek !== ""
    ) {
      var data = state.mereks.find(function (item) {
        return (
          item.vehicle_brand_name.toUpperCase() === payload.merek.toUpperCase()
        );
      });
    ...
    if (
      ...
      var data = state.tipes.find(function (item) {
        return (
          item.vehicle_varian_type_name.toUpperCase() ===
      ...
      if (data) {
        ...
        const hasilFilter = state.tipes.filter((f) =>
          [state.datakendaraan.idmerek].includes(f.vehicle_brand_id)
        );
    ...
    if (
      payload.idvarian !== null &&
      payload.idvarian !== undefined &&
      payload.idvarian !== ""
    ) {
      const hasilFilter = state.varians.filter((f) =>
        [state.datakendaraan.idtipe].includes(f.vehicle_varian_type_id)
      );
      state.varians = hasilFilter;
    }
    if (
      payload.jenis !== null &&
      payload.jenis !== undefined &&
      payload.jenis !== ""
    ) {
      var data = state.jenis.find(function (item) {
        return (
          item.vehicle_type_name.toUpperCase() === payload.jenis.toUpperCase()
        );
      });
      if (data) {
    ...
    if (
      payload.bahanbakar !== null &&
      payload.bahanbakar !== undefined &&
      payload.bahanbakar !== ""
    ) {
      var data = state.fuels.find(function (item) {
        return (
          item.fuel_name.toUpperCase() === payload.bahanbakar.toUpperCase()
        );
      });
      if (data) {
    ...
    if (
      payload.kelasjalanterendah !== null &&
      payload.kelasjalanterendah !== undefined &&
      payload.kelasjalanterendah !== ""
    ) {
    ...
    if (
      payload.jumlah_sumbu === null ||
      payload.jumlah_sumbu == "" ||
      payload.jumlah_sumbu == 0 ||
      payload.jumlah_sumbu == "0"
    ) {
      if (state.datakendaraan.jaraksumbu11_12 > 0) {
        ...
        if (
          state.datakendaraan.a === null ||
          state.datakendaraan.a == "" ||
          state.datakendaraan.a == "0"
        ) {
  ...
  ASSIGN_FORMNOUJI(state, payload) {
    ...
    if (
      ...
      const hasilFilter = state.varians.filter((f) =>
        [state.pendaftaran.idtipe].includes(f.vehicle_varian_type_id)
      );
      state.varians = hasilFilter;
    }
    if (
...
export const actions = {
    ...
    return new Promise((resolve, reject) => {
      DatakendaraanService.getDatakendaraan(payload)
        ...
        .catch((error) => {
          ...
          } else if (new String(error).includes(422)) {
            Swal.fire({
              icon: "error",
              title: "Oops...",
  ...
  getKelurahans({ commit, state }, payload) {
    return DatakendaraanService.getKelurahans(
      state.datakendaraan.kota,
      payload
    ).then((response) => {
  ...
  async getTipes({ commit, state }) {
    return await DatakendaraanService.getTipes(
      state.datakendaraan.idmerek
    ).then((response) => {
  ...
  async getVarians({ commit, state }) {
    return await DatakendaraanService.getVarians(
      state.datakendaraan.idtipe
    ).then((response) => {
  ...
  async getSubJenis({ commit, state }) {
    return await DatakendaraanService.getSubJenis(
      state.datakendaraan.idjenis
    ).then((response) => {
